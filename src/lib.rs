...
pub fn compute_modbus_crc(data: &[u8]) -> u16 {
    ...
    for chunk in chunks {
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(chunk[0])) & 0xFF) as usize];
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(chunk[1])) & 0xFF) as usize];
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(chunk[2])) & 0xFF) as usize];
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(chunk[3])) & 0xFF) as usize];
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(chunk[4])) & 0xFF) as usize];
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(chunk[5])) & 0xFF) as usize];
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(chunk[6])) & 0xFF) as usize];
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(chunk[7])) & 0xFF) as usize];
    }

    for &b in remainder {
        crc = (crc >> 8) ^ CRC_TABLE[((crc ^ u16::from(b)) & 0xFF) as usize];
    }
    crc
}
...
pub fn compute_batch_crcs_optimized(data: &[u8], iterations: u64, use_optimized: bool) -> u16 {
    ...
    let crcs: Vec<u16> = (0..num_batches)
        .into_par_iter()
        .map(|batch_idx| {
            let batch_iterations = if batch_idx == num_batches - 1 {
                iterations - (batch_idx * BATCH_SIZE)
            } else {
                BATCH_SIZE
            };
    ...
    *crcs.last().unwrap_or(&0)
}

pub fn parse_hex_input(hex_string: &str) -> Result<Vec<u8>, String> {
    let mut result = Vec::with_capacity(128);
    
    for part in hex_string.trim().split_whitespace() {
    ...
    if result.len() > 256 {
        return Err("Sekwencja nie może zawierać więcej niż 256 bajtów.".to_string());
    }
...
impl CrcResult {
    pub fn new(crc_value: u16, duration_ms: f64) -> Self {
        Self {
            crc_value,
            duration_ms,
            crc_lsb: (crc_value & 0xFF) as u8,
            crc_msb: (crc_value >> 8) as u8,
        }
    }
} 